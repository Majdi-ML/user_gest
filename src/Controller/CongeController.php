...
class CongeController extends AbstractController
{
    ...
    public function index(CongeRepository $congeRepository): Response
    {
        ...
        foreach ($conges as $conge) {
            $year = $conge->getDateDebut() ? $conge->getDateDebut()->format('Y') : 'Unknown';
            $employe = $conge->getEmploye();
            $dataByYear[$year][$conge->getId()] = [
                'id' => $conge->getId(),
                'date_debut' => $conge->getDateDebut() ? $conge->getDateDebut()->format('Y-m-d') : null,
                'date_fin' => $conge->getDateFin() ? $conge->getDateFin()->format('Y-m-d') : null,
                'motif' => $conge->getMotif(),
                'attachedfile' => $conge->getAttachedFile() ? true : false,
                'responsable' => $employe ? $employe->getNom() . ' ' . $employe->getPrenom() : null,
            ];
        }
